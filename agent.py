...
8. Push notification copy is a DIFFERENT medium from the in-app card and must be written
   separately, never reused from it. A lock-screen notification is glanceable: read in about
   one second, truncated hard, and sitting among personal messages — so the card's "sentence 1
   names the habit, sentence 2 invites them" structure reads robotic there. Write push_title
   and push_body to these rules instead:
   - Sentence case means: capitalise the FIRST letter, then normal prose capitalisation.
   ...
   - push_body: under 110 characters, at most two short clauses. It MUST still carry the lead
     ...
     Repeating "full refund if it's not right" verbatim across categories is exactly the
     template-stamped feel these fields exist to remove; vary the wording and the emphasis.
   - Sound like someone who noticed something, not a system that ran a query. Vary the opening
     across categories; never fall into one repeated template.
   - The ABSOLUTE ban on invented numbers, ratings, counts and percentages (rule 5) applies
     here too. Do not invent delivery times, discounts, prices, or buyer counts.
...
# Hook angles for push_title (agent.py rule 8). Each nudge is generated by an INDEPENDENT
# Groq call, so the model cannot see what the other users in a batch were given and converges
...
def _push_angle(profile):
    ...
    return PUSH_ANGLES[int(hashlib.md5(key.encode()).hexdigest(), 16) % len(PUSH_ANGLES)]
...
For push_title specifically (rule 8), use this angle for THIS user: {push_angle or _push_angle(profile)}.
Other users are being written with different angles, so do not fall back to a generic
"your <category> run is missing something" phrasing.
    ...
    resp = client.chat.completions.create(
        ...
        messages=[
            ...
            {"role": "user", "content": _build_user_prompt(profile, theme, match_reason,
                                                           exclude_category, forced_category,
                                                           push_angle)},
        ],
    )
